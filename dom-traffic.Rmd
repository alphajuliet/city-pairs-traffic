...
read_traffic <- function () {
  read_csv("data/audomcitypairs-20180406.csv")
}
...
```{r}
busiest_routes <- function (df, n = 20, year = 2017) { 
  df %>%
    filter(Year == year) %>%
...
dom %>%
  ...
  coord_flip() +
  labs(
    title = paste("Busiest routes in", year), 
    x = "Journey", 
    y = "Passengers") +
  theme_minimal() +
...
dom %>%
  ...
  labs(
    ...
    subtitle = paste("Plotted against", year, "passenger trips"),
    y = "Number of trips", 
    x = "Distance (km)") +
  theme_minimal()
```
